...
object program_8 extends Matchers{
  ...
  /**
    * 还可以通过提供某些但不是全部需要的参数表达一个部分应用函数
    * 这个部分应用函数产生的类的apply()函数就只带一个参数，相当于另外两个参数变成了默认函数
    */
  val b = sum(1, _: Int, 3)
  ...
  // 如果函数为一个输入定义了操作，那就返回 Some[A]，否则返回None。
  //要这么做的话，可以在给定的偏函数上调用 lift 方法得到一个普通的函数，这个函数返回Option。反过来，
  //如果有一个返回 Option 的普通函数f ，也可以调用Function.unlift(f)来得到一个偏函数。
